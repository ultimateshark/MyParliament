	...
	def wrap(*args, **kwargs):
		try:
			...
			if len(registered)>0:
				...
				return "Not Enrolled PLEASE Enroll To this Course"
		except:
			return "Not Enrolled PLEASE Enroll To this Course"
	return wrap
...
@login_required
def Dashboard_courses(name):
	courses=Courses.query.limit(25).all()
	user=GetUserInfo()
	enrolled_course=user.registered_in_courses
	return render_template("dashboard_"+name+".html",enrolled_course=enrolled_course,courses=courses)
